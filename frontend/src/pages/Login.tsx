...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 paper-texture relative overflow-hidden">
      ...
      {/* Coffee cup watermark / steam */}
      <div className="fixed -bottom-10 -right-10 w-44 h-44 rounded-full border-[6px] border-coffee-stain/10 pointer-events-none select-none rotate-12" />
      <div className="fixed bottom-24 right-16 w-32 h-32 rounded-full border border-coffee-stain/5 pointer-events-none select-none" />

      {/* Card Wrapper Container to prevent clipping of washi tape */}
      <div className="relative w-full max-w-md mt-6">
        {/* Washi tape decoration */}
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-28 h-7 bg-washi-pink/50 -rotate-1 rounded-sm shadow-sm border-x-2 border-dashed border-washi-pink/30 flex items-center justify-center z-20">
        ...
        >
          ...
          <div className="text-center mb-6">
            ...
            <p className="font-body italic text-xs text-muted-foreground">
              Sign in to check your saved library & swap logs
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between font-handwritten text-base text-muted-foreground pt-1">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input
                  type="checkbox"
                  checked={rememberMe}
                  onChange={(e) => setRememberMe(e.target.checked)}
                  className="accent-primary w-3.5 h-3.5 border-border rounded-sm bg-background"
                />
                <span className="mt-0.5">remember shelf</span>
              </label>
              <button
                type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 mt-2 rounded-sm bg-primary text-primary-foreground font-handwritten text-xl
            ...
            >
              ...
              <span>{loading ? "checking card..." : "Log in to shelf ✨"}</span>
            </button>
          </form>
